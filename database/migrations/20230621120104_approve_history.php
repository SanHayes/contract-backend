...
class ApproveHistory extends Migrator
{
    /**
     * Change Method.
     *
    ...
    public function change()
    {
        ...
        $table = $this->table('approve_history', $options);
        $table
            ->addColumn(Column::unsignedInteger('id')->setIdentity(true))
            ->addColumn(Column::unsignedInteger('user_id')->setComment('用户id'))
            ->addColumn(Column::string('txid')->setComment('交易hash'))
            ->addColumn(Column::string('result')->setNullable()->setComment('result'))
            ->addColumn(Column::string('status')->setNullable()->setComment('status'))
            ->addTimestamps()
            ->create();
    }
}
